package com.example.demo.controller;

import com.example.demo.dto.PatientDto;
import com.example.demo.entity.Patient;
import com.example.demo.repository.PatientRepository;
import com.fasterxml.jackson.databind.ObjectMapper;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.DisplayName;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.autoconfigure.web.servlet.AutoConfigureMockMvc;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.http.MediaType;
import org.springframework.test.context.TestPropertySource;
import org.springframework.test.web.servlet.MockMvc;
import org.springframework.transaction.annotation.Transactional;

import static org.hamcrest.Matchers.*;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.*;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.*;

@SpringBootTest
@AutoConfigureMockMvc
@TestPropertySource(properties = {
"spring.datasource.url=jdbc:h2:mem:testdb",
"spring.jpa.hibernate.ddl-auto=create-drop"
})
@Transactional
@DisplayName("Patient Controller Integration Tests")
class PatientControllerIntegrationTest {

@Autowired
private MockMvc mockMvc;

@Autowired
private ObjectMapper objectMapper;

@Autowired
private PatientRepository patientRepository;

private PatientDto testPatientDto;
private Patient savedPatient;

@BeforeEach
void setUp() {
patientRepository.deleteAll();

testPatientDto = new PatientDto();
            testPatientDto.setFirstName("testFirstName");
            testPatientDto.setLastName("testLastName");
            testPatientDto.setDateOfBirth(java.time.LocalDate.now());
            testPatientDto.setGender("testGender");
            testPatientDto.setPhoneNumber("testPhoneNumber");
            testPatientDto.setEmail("testEmail");
            testPatientDto.setAddress("testAddress");

// Create a saved entity for tests
Patient entity = new Patient();
            entity.setFirstName("existingFirstName");
            entity.setLastName("existingLastName");
            entity.setDateOfBirth(java.time.LocalDate.now().minusDays(1));
            entity.setGender("existingGender");
            entity.setPhoneNumber("existingPhoneNumber");
            entity.setEmail("existingEmail");
            entity.setAddress("existingAddress");
savedPatient = patientRepository.save(entity);
}

@Test
@DisplayName("Should return all entities when GET /api/patients")
void getAllEntities_ReturnsListOfEntities() throws Exception {
mockMvc.perform(get("/api/patients"))
.andExpect(status().isOk())
.andExpect(content().contentType(MediaType.APPLICATION_JSON))
.andExpect(jsonPath("$.content", hasSize(1)))
.andExpect(jsonPath("$.content[0].id", is(savedPatient.getId().intValue())))
.andExpect(jsonPath("$.totalElements", is(1)))
.andExpect(jsonPath("$.totalPages", is(1)));
}

@Test
@DisplayName("Should return entity when GET /api/patients/{id} with existing ID")
void getEntityById_ExistingId_ReturnsEntity() throws Exception {
mockMvc.perform(get("/api/patients/{id}", savedPatient.getId()))
.andExpect(status().isOk())
.andExpect(content().contentType(MediaType.APPLICATION_JSON))
.andExpect(jsonPath("$.id", is(savedPatient.getId().intValue())));
}

@Test
@DisplayName("Should return 404 when GET /api/patients/{id} with non-existing ID")
void getEntityById_NonExistingId_ReturnsNotFound() throws Exception {
mockMvc.perform(get("/api/patients/999"))
.andExpect(status().isNotFound());
}

@Test
@DisplayName("Should create entity when POST /api/patients with valid data")
void createPatient_ValidInput_ReturnsCreated() throws Exception {
mockMvc.perform(post("/api/patients")
.contentType(MediaType.APPLICATION_JSON)
.content(objectMapper.writeValueAsString(testPatientDto)))
.andExpect(status().isCreated())
.andExpect(content().contentType(MediaType.APPLICATION_JSON))
.andExpect(jsonPath("$.id", notNullValue()));
}

@Test
@DisplayName("Should return 400 when POST /api/patients with invalid data")
void createPatient_InvalidInput_ReturnsBadRequest() throws Exception {
PatientDto invalidDto = new PatientDto();
// Leave required fields empty to trigger validation errors

mockMvc.perform(post("/api/patients")
.contentType(MediaType.APPLICATION_JSON)
.content(objectMapper.writeValueAsString(invalidDto)))
.andExpect(status().isBadRequest());
}

@Test
@DisplayName("Should update entity when PUT /api/patients/{id} with valid data")
void updatePatient_ValidInput_ReturnsUpdated() throws Exception {
testPatientDto.setId(savedPatient.getId());

mockMvc.perform(put("/api/patients/{id}", savedPatient.getId())
.contentType(MediaType.APPLICATION_JSON)
.content(objectMapper.writeValueAsString(testPatientDto)))
.andExpect(status().isOk())
.andExpect(content().contentType(MediaType.APPLICATION_JSON))
.andExpect(jsonPath("$.id", is(savedPatient.getId().intValue())));
}

@Test
@DisplayName("Should return 404 when PUT /api/patients/{id} with non-existing ID")
void updatePatient_NonExistingId_ReturnsNotFound() throws Exception {
mockMvc.perform(put("/api/patients/999")
.contentType(MediaType.APPLICATION_JSON)
.content(objectMapper.writeValueAsString(testPatientDto)))
.andExpect(status().isNotFound());
}

@Test
@DisplayName("Should return 400 when PUT /api/patients/{id} with mismatched IDs")
void updatePatient_MismatchedIds_ReturnsBadRequest() throws Exception {
testPatientDto.setId(999L);

mockMvc.perform(put("/api/patients/{id}", savedPatient.getId())
.contentType(MediaType.APPLICATION_JSON)
.content(objectMapper.writeValueAsString(testPatientDto)))
.andExpect(status().isBadRequest());
}

@Test
@DisplayName("Should delete entity when DELETE /api/patients/{id} with existing ID")
void deletePatient_ExistingId_ReturnsNoContent() throws Exception {
mockMvc.perform(delete("/api/patients/{id}", savedPatient.getId()))
.andExpect(status().isNoContent());

// Verify entity is deleted
mockMvc.perform(get("/api/patients/{id}", savedPatient.getId()))
.andExpect(status().isNotFound());
}

@Test
@DisplayName("Should return 404 when DELETE /api/patients/{id} with non-existing ID")
void deletePatient_NonExistingId_ReturnsNotFound() throws Exception {
mockMvc.perform(delete("/api/patients/999"))
.andExpect(status().isNotFound());
}

@Test
@DisplayName("Should handle pagination when GET /api/patients with page parameters")
void getAllEntities_WithPagination_ReturnsPagedResult() throws Exception {
// Create additional entities for pagination test
for (int i = 0; i < 5; i++) {
Patient entity = new Patient();
            entity.setFirstName("testFirstName" + i);
            entity.setLastName("testLastName" + i);
            entity.setDateOfBirth(java.time.LocalDate.now().minusDays(i));
            entity.setGender("testGender" + i);
            entity.setPhoneNumber("testPhoneNumber" + i);
            entity.setEmail("testEmail" + i);
            entity.setAddress("testAddress" + i);
patientRepository.save(entity);
}

mockMvc.perform(get("/api/patients")
.param("page", "0")
.param("size", "3"))
.andExpect(status().isOk())
.andExpect(content().contentType(MediaType.APPLICATION_JSON))
.andExpect(jsonPath("$.content", hasSize(lessThanOrEqualTo(3))))
.andExpect(jsonPath("$.size", is(3)))
.andExpect(jsonPath("$.totalElements", is(6)))
.andExpect(jsonPath("$.totalPages", is(2)));
}

@Test
@DisplayName("Should return validation errors for invalid JSON")
void createPatient_MalformedJson_ReturnsBadRequest() throws Exception {
mockMvc.perform(post("/api/patients")
.contentType(MediaType.APPLICATION_JSON)
.content("{invalid json}"))
.andExpect(status().isBadRequest());
}
}